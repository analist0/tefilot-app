...
import { Heart, BookOpen, ChevronRight, Home, CheckCircle2, Circle } from "lucide-react"
import { BRACHOT_STRUCTURE, type TefilaSection } from "@/lib/sefaria/tefilot"

export default function BrachotPage() {
  const [completedSections, setCompletedSections] = useState<Set<string>>(new Set())

  useEffect(() => {
    const saved = localStorage.getItem("brachot_completed")
    if (saved) {
  ...
  const toggleSection = (title: string) => {
    ...
    localStorage.setItem("brachot_completed", JSON.stringify([...newCompleted]))
  }
  ...
  const totalSections = BRACHOT_STRUCTURE.reduce((acc, section) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background via-pink-50/30 dark:via-pink-950/10 to-background" dir="rtl">
      {/* Header */}
      <div className="bg-gradient-to-br from-pink-400 via-rose-500 to-red-500 text-white py-12 px-4">
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-4xl md:text-5xl font-bold font-['Frank_Ruhl_Libre']">ברכות</h1>
              <p className="text-xl opacity-90 mt-2">ברכות יומיות ומצוות</p>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
          ...
          <CardContent className="pt-6">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2">
                <h3 className="font-bold text-lg">ברכות יומיות</h3>
                <p className="text-muted-foreground text-sm leading-relaxed">
                  אוסף ברכות לכל צורך - ברכות השחר, ברכות הנהנין (על אוכלים ומשקאות),
                  ברכת המזון, וברכות על קיום מצוות. כל ברכה מביאה את האדם לקשר עם הקב"ה.
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <div className="space-y-4">
          <h2 className="text-2xl font-bold font-['Frank_Ruhl_Libre']">סוגי הברכות</h2>
          {BRACHOT_STRUCTURE.map((section, index) => renderSection(section, index))}
        </div>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">התחל מחדש את מעקב הברכות</p>
                </div>
                <Button
                  variant="outline"
                  onClick={() => {
                    setCompletedSections(new Set())
                    localStorage.removeItem("brachot_completed")
                  }}
                >
                  אפס
                </Button>
