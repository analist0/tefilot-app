...
import { Moon, BookOpen, ChevronRight, Home, CheckCircle2, Circle } from "lucide-react"
import { MAARIV_STRUCTURE, type TefilaSection } from "@/lib/sefaria/tefilot"

export default function MaarivPage() {
  const [completedSections, setCompletedSections] = useState<Set<string>>(new Set())

  useEffect(() => {
    const saved = localStorage.getItem("maariv_completed")
    if (saved) {
  ...
  const toggleSection = (title: string) => {
    ...
    localStorage.setItem("maariv_completed", JSON.stringify([...newCompleted]))
  }
  ...
  const totalSections = MAARIV_STRUCTURE.reduce((acc, section) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background via-indigo-50/30 dark:via-indigo-950/10 to-background" dir="rtl">
      {/* Header */}
      <div className="bg-gradient-to-br from-indigo-500 via-purple-600 to-violet-700 text-white py-12 px-4">
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-4xl md:text-5xl font-bold font-['Frank_Ruhl_Libre']">תפילת ערבית</h1>
              <p className="text-xl opacity-90 mt-2">תפילת הערב - מעריב</p>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
          ...
          <CardContent className="pt-6">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2">
                <h3 className="font-bold text-lg">תפילת ערבית</h3>
                <p className="text-muted-foreground text-sm leading-relaxed">
                  תפילת ערבית (מעריב) היא תפילת הערב, וכוללת קריאת שמע עם ברכותיה, עמידה ועלינו.
                  זו התפילה היחידה בה אומרים "ברוך אתה ה' המעריב ערבים" - המחליף יום בלילה.
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <div className="space-y-4">
          ...
          {MAARIV_STRUCTURE.map((section, index) => renderSection(section, index))}
        </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    localStorage.removeItem("maariv_completed")
                  }}
                >
                  אפס
                </Button>
